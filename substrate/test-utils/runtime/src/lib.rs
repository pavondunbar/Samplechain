...
pub mod currency {
	...
	pub const SMPL: Balance = 100 * CENTS;
}

parameter_types! {
	pub const ExistentialDeposit: Balance = 1 * currency::SMPL;
			...
			get_preset::<RuntimeGenesisConfig>(name, |name| {
				let patch = match name.try_into() {
					Ok("staging") => {
						...
						json!({
							"balances": {
								"balances": endowed_accounts.into_iter().map(|k| (k, 10 * currency::SMPL)).collect::<Vec<_>>(),
							},
							"substrateTest": {
								"authorities": [
