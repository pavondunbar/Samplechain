...
use samplechain_runtime::{
	constants::currency::SMPL, AccountId, BalancesCall, CheckedExtrinsic, MinimumPeriod,
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		let signed = self.keyring.sign(
			CheckedExtrinsic {
				signed: Some((
					sender,
					signed_extra(0, samplechain_runtime::ExistentialDeposit::get() + 1),
				)),
				function: match self.content.block_type {
					BlockType::RandomTransfersKeepAlive =>
						RuntimeCall::Balances(BalancesCall::transfer_keep_alive {
							dest: sp_runtime::MultiAddress::Id(receiver),
							value: samplechain_runtime::ExistentialDeposit::get() + 1,
						}),
					BlockType::RandomTransfersReaping => {
						RuntimeCall::Balances(BalancesCall::transfer_allow_death {
							...
							value: 100 * SMPL -
								(samplechain_runtime::ExistentialDeposit::get() - 1),
						})
					},
					BlockType::Noop =>
...
impl sp_runtime::BuildStorage for BenchKeyring {
	fn assimilate_storage(&self, storage: &mut sp_core::storage::Storage) -> Result<(), String> {
		storage.top.insert(
			...
			samplechain_runtime::wasm_binary_unwrap().into(),
		);
