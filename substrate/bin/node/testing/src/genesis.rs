...
pub fn config_endowed(extra_endowed: Vec<AccountId>) -> RuntimeGenesisConfig {
	let mut endowed = vec![
		(alice(), 111 * SMPL),
		(bob(), 100 * SMPL),
		(charlie(), 100_000_000 * SMPL),
		(dave(), 111 * SMPL),
		(eve(), 101 * SMPL),
		(ferdie(), 100 * SMPL),
	];

	endowed.extend(extra_endowed.into_iter().map(|endowed| (endowed, 100 * SMPL)));
	...
	RuntimeGenesisConfig {
		...
		staking: StakingConfig {
			stakers: vec![
				(dave(), dave(), 111 * SMPL, StakerStatus::Validator),
				(eve(), eve(), 100 * SMPL, StakerStatus::Validator),
				(ferdie(), ferdie(), 100 * SMPL, StakerStatus::Validator),
			],
			validator_count: 3,
