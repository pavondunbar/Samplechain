...
use futures::prelude::*;
use samplechain_runtime::{RuntimeApi,TransactionConverter};
...
pub fn create_extrinsic(
	...
	function: impl Into<samplechain_runtime::RuntimeCall>,
	nonce: Option<u32>,
) -> samplechain_runtime::UncheckedExtrinsic {
	...
	let period = samplechain_runtime::BlockHashCount::get()
		.checked_next_power_of_two()
		.map(|c| c / 2)
		.unwrap_or(2) as u64;
	let tip = 0;
	let extra: samplechain_runtime::SignedExtra =
		(
			frame_system::CheckNonZeroSender::<samplechain_runtime::Runtime>::new(),
			frame_system::CheckSpecVersion::<samplechain_runtime::Runtime>::new(),
			frame_system::CheckTxVersion::<samplechain_runtime::Runtime>::new(),
			frame_system::CheckGenesis::<samplechain_runtime::Runtime>::new(),
			frame_system::CheckEra::<samplechain_runtime::Runtime>::from(generic::Era::mortal(
				period,
				best_block.saturated_into(),
			)),
			frame_system::CheckNonce::<samplechain_runtime::Runtime>::from(nonce),
			frame_system::CheckWeight::<samplechain_runtime::Runtime>::new(),
			pallet_skip_feeless_payment::SkipCheckIfFeeless::from(
				pallet_asset_conversion_tx_payment::ChargeAssetTxPayment::<
					samplechain_runtime::Runtime,
				>::from(tip, None),
			),
			frame_metadata_hash_extension::CheckMetadataHash::new(false),
		);

	let raw_payload = samplechain_runtime::SignedPayload::from_raw(
		function.clone(),
		extra.clone(),
		(
			(),
			samplechain_runtime::VERSION.spec_version,
			samplechain_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	samplechain_runtime::UncheckedExtrinsic::new_signed(
		function,
		sp_runtime::AccountId32::from(sender.public()).into(),
		samplechain_runtime::Signature::Sr25519(signature),
		extra,
	)
}
