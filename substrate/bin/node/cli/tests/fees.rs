...
fn new_account_info(free_ARGO: u128) -> Vec<u8> {
	frame_system::AccountInfo {
		nonce: 0u32,
		consumers: 0,
		providers: 1,
		sufficients: 0,
		data: (free_ARGO * SMPL, 0 * SMPL, 0 * SMPL, 1u128 << 127),
	}
	.encode()
}
...
fn transaction_fee_is_correct() {
	...
	t.execute_with(|| {
		assert_eq!(Balances::total_balance(&bob()), (10 + 69) * SMPL);
		...
		let mut balance_alice = (100 - 69) * SMPL;
