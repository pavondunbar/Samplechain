...
use samplechain_runtime::{constants::currency::*, BalancesCall};
...
fn extrinsic_set_time(now: u64) -> OpaqueExtrinsic {
	samplechain_runtime::UncheckedExtrinsic {
		signature: None,
		function: samplechain_runtime::RuntimeCall::Timestamp(pallet_timestamp::Call::set { now }),
	}
	.into()
}
...
fn prepare_benchmark(client: &FullClient) -> (usize, Vec<OpaqueExtrinsic>) {
	...
	for nonce in 0.. {
		let extrinsic: OpaqueExtrinsic = create_extrinsic(
			client,
			src.clone(),
			BalancesCall::transfer_allow_death { dest: dst.clone(), value: 1 * SMPL },
			Some(nonce),
		)
		.into();
